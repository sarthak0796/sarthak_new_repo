...
def execute_query(prj_nm,paths_data:str,task_name:str,json_data,run_id,restart_point=None,query_iteration=None,task_iteration=None):
    try:
        ...
        if restart_mode == "normal" or restart_mode == "":
            task_logger.info("task running in normal restart mode")
            ...
            task_logger.info("task running in skip restart mode")
            sorted_queries = [query for query in sorted_queries if int(query["seq_no"]) > int(restart_point)]
            #task_logger.info(sorted_queries)
            ...
            task_logger.info("task running in begin restart mode")
            if restart_point == 1:
                itervalue = 1
            else:
                itervalue = task_iteration + 1
    ...
    if restart_mode !="no restart":
        ...
        if result != []:
            ...
            if audit_value_task != "COMPLETED":
                ...
                # if result == [] :
                #     task_logger.info("This is the new task so it will run for the first time ")
                # else:
